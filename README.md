# Onymos-Assessment
This is a private repository im using for an assessment for the company Onymos

The current code simulates a Stock Exchange with 2 functions, addOrder and matchOrder. The task I was given to accomplish is posted below.
...
Known Bugs: The program will currently match orders for 0 shares because when an order is completed, it is not removed from the list of Orders. I was not asked to make a function that removes orders so I did not do so, but that would fix the issue.
